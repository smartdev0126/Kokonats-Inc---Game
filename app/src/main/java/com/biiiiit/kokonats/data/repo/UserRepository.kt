...
class UserRepository {
    ...
    suspend fun checkUsername(username: String): KoKoResponse<Boolean> {
        val result = userApi.checkUsername(username)
        return KoKoResponse(NET_SUCCESS, "", true)
    }

    suspend fun registerUserDeviceToken(token: String): KoKoResponse<Boolean> {
        val result = userApi.registerUserDevice(platform = "android", deviceToken = token)
        return KoKoResponse(NET_SUCCESS, "", true)
    }
}